...
class TestFullAgentWorkflow:
    ...
    async def test_complete_agent_task_workflow(
        ...
        DatabaseTestHelper(test_session)
        ...
        with patch(
            ...
            side_effect=mock_websocket_broadcast,
        ):
            ...
            db_agent = await test_session.execute(
            ...
            task_data = TestDataGenerator.task_data(
                ...
                metadata={"test_type": "integration", "workflow_step": "execution"},
            )
            ...
            status_response = await test_client.patch(
                f"/api/v1/agents/{agent_id}/status", json=status_update_data
            )
            ...
            status_update_messages = ws_helper.get_messages_by_type(
            ...
            assert (
                len(
                    [
                        msg
                        for msg in task_update_messages
            ...
            for progress in progress_steps:
                progress_response = await test_client.patch(
                    f"/api/v1/tasks/{task_id}/progress", json={"progress": progress}
                )
            ...
            assert (
                updated_metrics["avg_response_time"]
            ...
            idle_response = await test_client.patch(
                f"/api/v1/agents/{agent_id}/status", json=idle_status_data
            )
            ...
            final_agent = await test_session.execute(
                select(Agent).where(Agent.id == agent_id)
            )
            ...
            final_task = await test_session.execute(
            ...
            for message_type in expected_message_types:
                ...
                assert (
                    len(messages) >= 1
                ), f"Expected at least one {message_type} message"
            ...
            delete_agent_response = await test_client.delete(
            ...
            deleted_agent = await test_session.execute(
                select(Agent).where(Agent.id == agent_id)
            )
            deleted_task = await test_session.execute(
                select(Task).where(Task.id == task_id)
            )
    ...
    async def test_error_handling_workflow(
        ...
        with patch(
            ...
            task = await api_helper.create_task(
                title="Error Test Task", agent_id=agent_id
            )
            task_id = task["id"]
            ...
            failure_response = await test_client.patch(
                f"/api/v1/tasks/{task_id}/fail", json=failure_data
            )
            ...
            error_response = await test_client.patch(
                f"/api/v1/agents/{agent_id}/status", json=error_status
            )
            ...
            failure_metrics = {"tasks_failed": 1}

            metrics_response = await test_client.patch(
                f"/api/v1/agents/{agent_id}/metrics", json=failure_metrics
            )
            assert metrics_response.status_code == 200
    ...
    async def test_concurrent_operations(self, test_client: AsyncClient, test_session):
        ...
        with patch(
            ...
            for i in range(5):
                agent_data = TestDataGenerator.agent_data(
                    name=f"concurrent-agent-{i}", agent_type="concurrent-test"
                )
            ...
            for i, agent_id in enumerate(agent_ids):
                task_data = TestDataGenerator.task_data(
                    title=f"Concurrent Task {i}", agent_id=agent_id
                )
            ...
            for agent_id, task_id in zip(agent_ids, task_ids):
                status_data = {"status": "busy", "current_task_id": task_id}
