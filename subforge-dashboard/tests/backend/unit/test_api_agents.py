...
class TestAgentAPI:
    ...
    async def test_get_agents_with_filters(
    ...
    async def test_get_agents_pagination(self, test_client: AsyncClient, test_session):
        ...
        agents = [
            Agent(name=f"agent{i}", agent_type="test", status="idle") for i in range(5)
        ]
    ...
    async def test_create_agent_duplicate(
        self, test_client: AsyncClient, test_agent, sample_agent_data
    ):
    ...
    async def test_create_agent_invalid_data(self, test_client: AsyncClient):
        ...
        invalid_data = {"name": "", "agent_type": "test"}  # Empty name
    ...
    async def test_update_agent_status(self, test_client: AsyncClient, test_agent):
        ...
        with patch(
            ...
            response = await test_client.patch(
                f"/api/v1/agents/{test_agent.id}/status", json=status_update
            )
    ...
    async def test_update_agent_metrics(self, test_client: AsyncClient, test_agent):
        ...
        with patch(
            ...
            response = await test_client.patch(
                f"/api/v1/agents/{test_agent.id}/metrics", json=metrics_update
            )
    ...
    async def test_agent_heartbeat(self, test_client: AsyncClient, test_agent):
        ...
        response = await test_client.post(
            f"/api/v1/agents/{test_agent.id}/heartbeat", json=heartbeat_data
        )
...
class TestAgentAPIValidation:
    ...
    async def test_create_agent_missing_required_fields(self, test_client: AsyncClient):
        ...
        # Missing name
        incomplete_data = {"agent_type": "test", "status": "idle"}

        response = await test_client.post("/api/v1/agents/", json=incomplete_data)
        assert response.status_code == 422

        # Missing agent_type
        incomplete_data = {"name": "test", "status": "idle"}
    ...
    async def test_update_agent_invalid_uuid(self, test_client: AsyncClient):
        ...
        response = await test_client.put(
            "/api/v1/agents/invalid-uuid", json=update_data
        )
        assert response.status_code == 422

    async def test_agent_heartbeat_invalid_data(
        ...
        response = await test_client.post(
            f"/api/v1/agents/{test_agent.id}/heartbeat", json=invalid_data
        )
        assert response.status_code == 422

        # Invalid timestamp format
        invalid_data = {"timestamp": "invalid-timestamp", "status": "active"}
...
class TestAgentAPIWebSocket:
    ...
    async def test_agent_creation_broadcasts_websocket(
        self, test_client: AsyncClient, sample_agent_data
    ):
    ...
    async def test_agent_update_broadcasts_websocket(
        self, test_client: AsyncClient, test_agent
    ):
    ...
    async def test_agent_status_update_broadcasts_websocket(
        self, test_client: AsyncClient, test_agent
    ):
        ...
        with patch(
            ...
            response = await test_client.patch(
                f"/api/v1/agents/{test_agent.id}/status", json=status_data
            )
            assert response.status_code == 200
    ...
    async def test_agent_metrics_update_broadcasts_websocket(
        self, test_client: AsyncClient, test_agent
    ):
        ...
        with patch(
            ...
            response = await test_client.patch(
                f"/api/v1/agents/{test_agent.id}/metrics", json=metrics_data
            )
            assert response.status_code == 200
    ...
    async def test_agent_deletion_broadcasts_websocket(
        self, test_client: AsyncClient, test_agent
    ):
