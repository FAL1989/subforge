...
class TestTaskModel:
    ...
    async def test_task_with_agent(self, test_session, test_agent):
        ...
        task = Task(
            title="Test Task", description="A test task", agent_id=test_agent.id
        )
...
class TestWorkflowModel:
    ...
    async def test_workflow_creation(self, test_session):
        ...
        workflow = Workflow(
            name="test-workflow", description="A test workflow", status="pending"
        )
    ...
    async def test_workflow_with_steps(self, test_session):
        ...
        workflow = Workflow(
            name="test-workflow", description="Test workflow with steps", steps=steps
        )
...
class TestSystemMetricsModel:
    ...
    async def test_system_metrics_creation(self, test_session):
        ...
        metrics = SystemMetrics(
            metric_name="cpu_usage", metric_value=75.5, unit="percent"
        )
...
class TestModelRelationships:
    ...
    async def test_agent_task_relationship(self, test_session):
        ...
        task = Task(title="Test Task", description="Test task", agent_id=agent.id)
        test_session.add(task)
        await test_session.commit()
        await test_session.refresh(task)
    ...
    async def test_workflow_task_relationship(self, test_session):
        ...
        task1 = Task(title="Task 1", description="First task", workflow_id=workflow.id)
        task2 = Task(title="Task 2", description="Second task", workflow_id=workflow.id)
