...
class TestSQLInjection:
    ...
    async def test_agent_name_sql_injection(
        self, test_client: AsyncClient, security_payloads
    ):
        """Test SQL injection in agent name field"""
        for payload in security_payloads["sql_injection"]:
            agent_data = {"name": payload, "agent_type": "test", "status": "idle"}
            ...
            assert response.status_code in [
                200,
                422,
            ]  # Valid creation or validation error
    ...
    async def test_agent_filter_sql_injection(
        self, test_client: AsyncClient, security_payloads
    ):
        """Test SQL injection in filter parameters"""
        for payload in security_payloads["sql_injection"]:
            # Test various filter parameters
            params = {"status": payload, "agent_type": payload, "skip": 0, "limit": 10}
    ...
    async def test_task_search_sql_injection(
        self, test_client: AsyncClient, security_payloads
    ):
        """Test SQL injection in task search parameters"""
        for payload in security_payloads["sql_injection"]:
            params = {"title": payload, "status": payload, "priority": payload}
...
class TestXSSProtection:
    ...
    async def test_agent_description_xss(
        self, test_client: AsyncClient, security_payloads
    ):
...
class TestInputValidation:
    ...
    async def test_json_payload_size_limit(self, test_client: AsyncClient):
        ...
        assert response.status_code in [
            ...
            422,
            400,
        ]  # Request too large or validation error
...
class TestWebSocketSecurity:
    ...
    def test_websocket_message_size_limit(self, sync_client):
        ...
        with sync_client.websocket_connect("/ws") as websocket:
            ...
            large_message = {"type": "test", "data": "A" * 100000}  # 100KB message
...
class TestDataLeakage:
    ...
    async def test_database_error_handling(self, test_client: AsyncClient):
        ...
        agent_data = {"name": "test-agent", "agent_type": "test", "status": "idle"}
...
class TestSecurityPerformance:
    ...
    async def test_regular_expression_dos(self, test_client: AsyncClient):
        ...
        malicious_patterns = ["a" * 1000 + "b", "(a+)+$", "^(a+)+$", "^((a+)+)+$"]

        for pattern in malicious_patterns:
            agent_data = {"name": pattern, "agent_type": "test", "status": "idle"}
