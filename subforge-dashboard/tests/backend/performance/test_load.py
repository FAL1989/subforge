...
class TestAPIPerformance:
    ...
    async def test_agent_list_performance(self, test_client: AsyncClient, test_session, performance_config):
    ...
    async def test_agent_creation_performance(self, test_client: AsyncClient, performance_config):
    ...
    async def test_agent_update_performance(self, test_client: AsyncClient, test_agent, performance_config):
    ...
    async def test_database_query_performance(self, test_client: AsyncClient, test_session, performance_config):
        ...
        for endpoint, description in test_cases:
            ...
            assert response_time < performance_config["max_response_time"] * 2, f"Too slow: {description}"
...
class TestConcurrentLoad:
    ...
    async def test_concurrent_agent_creation(self, test_client: AsyncClient, performance_config):
        ...
        # Assertions
        assert success_rate >= (1 - performance_config["acceptable_error_rate"]), f"Success rate too low: {success_rate}"
        assert avg_response_time < performance_config["max_response_time"] * 2, f"Average response time too high: {avg_response_time}"
    ...
    async def test_concurrent_agent_reads(self, test_client: AsyncClient, test_session, performance_config):
        ...
        assert avg_response_time < performance_config["max_response_time"], f"Average read time too high: {avg_response_time}"
    
    async def test_mixed_workload_performance(self, test_client: AsyncClient, test_session, performance_config):
        ...
        assert total_time < performance_config["test_duration"], "Mixed workload took too long"
...
class TestDatabasePerformance:
    ...
    async def test_complex_query_performance(self, test_session, performance_config):
        ...
        complex_queries = [
            ...
            select(Agent)
            .where(and_(Agent.success_rate > 80, Agent.status == 'active')),
...
class TestMemoryUsage:
    ...
    async def test_memory_usage_large_response(self, test_client: AsyncClient, test_session):
        ...
        for batch_start in range(0, num_agents, batch_size):
            ...
            for i in range(batch_start, min(batch_start + batch_size, num_agents)):
                agent = Agent(
                    ...
                    configuration={"key1": "value1", "key2": "value2", "numbers": list(range(10))}
                )
