...
class TestRunner:
    ...
    def check_dependencies(self) -> bool:
        ...
        try:
            subprocess.run([sys.executable, "-c", "import pytest, fastapi, sqlalchemy"], 
                         check=True, capture_output=True)
            print("✅ Python dependencies OK")
        except subprocess.CalledProcessError:
            print("❌ Missing Python dependencies. Run: pip install -r backend/requirements.txt")
            return False
    ...
    def run_e2e_tests(self, args: argparse.Namespace) -> bool:
        ...
        try:
            subprocess.run(["npx", "playwright", "install"], check=True, capture_output=True)
    ...
    def generate_report(self):
        ...
        for suite_name, results in self.results.items():
            ...
            print(f"{status} {suite_name.capitalize()}: {results['passed']} passed, {results['failed']} failed")
...
def main():
    ...
    parser.add_argument("--workers", type=int, help="Number of parallel workers for E2E")
