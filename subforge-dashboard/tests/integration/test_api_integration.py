...
class TestAgentAPIIntegration:
    ...
    async def test_complete_agent_lifecycle(self, test_client: AsyncClient, test_session):
        ...
        with patch('app.websocket.manager.websocket_manager.broadcast_json') as mock_broadcast:
            ...
            update_response = await test_client.put(f"/api/v1/agents/{agent_id}", json=update_data)
    ...
    async def test_agent_with_tasks_integration(self, test_client: AsyncClient, test_session):
    ...
    async def test_agent_metrics_integration(self, test_client: AsyncClient):
        ...
        for metrics in metrics_updates:
            with patch('app.websocket.manager.websocket_manager.broadcast_json') as mock_broadcast:
                ...
                expected_success_rate = (metrics["tasks_completed"] / 
...
class TestTaskAPIIntegration:
    ...
    async def test_complete_task_workflow(self, test_client: AsyncClient, test_session):
        ...
        # Create agent first
        agent_data = {"name": "workflow-agent", "agent_type": "workflow-test", "status": "idle"}
...
class TestWorkflowIntegration:
    ...
    async def test_workflow_with_tasks(self, test_client: AsyncClient, test_session):
        ...
        # Start workflow
        with patch('app.websocket.manager.websocket_manager.broadcast_json'):
            start_response = await test_client.patch(f"/api/v1/workflows/{workflow_id}/start")
            assert start_response.status_code == 200
        ...
        # Complete workflow
        with patch('app.websocket.manager.websocket_manager.broadcast_json'):
            complete_response = await test_client.patch(f"/api/v1/workflows/{workflow_id}/complete")
            assert complete_response.status_code == 200
...
class TestWebSocketIntegration:
    ...
    async def test_agent_updates_broadcast_websocket(self, test_client: AsyncClient):
        ...
        with patch('app.websocket.manager.websocket_manager') as mock_ws_manager:
            ...
            # Create agent
            agent_data = {"name": "ws-test-agent", "agent_type": "websocket-test", "status": "idle"}
            create_response = await test_client.post("/api/v1/agents/", json=agent_data)
            agent_id = create_response.json()["id"]
            ...
            # Update agent status
            status_data = {"status": "active", "current_task_title": "WebSocket test task"}
            await test_client.patch(f"/api/v1/agents/{agent_id}/status", json=status_data)
            ...
            await test_client.patch(f"/api/v1/agents/{agent_id}/metrics", json=metrics_data)
    ...
    async def test_task_updates_broadcast_websocket(self, test_client: AsyncClient):
        ...
        with patch('app.websocket.manager.websocket_manager') as mock_ws_manager:
            ...
            await test_client.patch(f"/api/v1/tasks/{task_id}/progress", json=progress_data)
...
class TestAPIPerformanceIntegration:
    ...
    async def test_bulk_operations_performance(self, test_client: AsyncClient, test_session):
        ...
        with patch('app.websocket.manager.websocket_manager.broadcast_json'):
            for agent_id in agent_ids:
                ...
                response = await test_client.put(f"/api/v1/agents/{agent_id}", json=update_data)
                assert response.status_code == 200
...
class TestDatabaseConsistency:
    ...
    async def test_transaction_consistency(self, test_client: AsyncClient, test_session):
        """Test database transaction consistency"""
        
        # Create agent and task in a workflow that should maintain consistency
        agent_data = {"name": "consistency-agent", "agent_type": "consistency-test", "status": "idle"}
        ...
        agent_result = await test_session.execute(select(Agent).where(Agent.id == agent_id))
    ...
    async def test_cascade_operations(self, test_client: AsyncClient, test_session):
        ...
        with patch('app.websocket.manager.websocket_manager.broadcast_json'):
            workflow_response = await test_client.post("/api/v1/workflows/", json=workflow_data)
        ...
        # Verify both exist
        workflow_result = await test_session.execute(select(Workflow).where(Workflow.id == workflow_id))
        task_result = await test_session.execute(select(Task).where(Task.id == task_id))
        ...
        with patch('app.websocket.manager.websocket_manager.broadcast_json'):
            ...
            assert delete_response.status_code in [200, 409]  # 409 if cascade not allowed
