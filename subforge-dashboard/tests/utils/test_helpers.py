...
class TestDataGenerator:
    ...
    def agent_data(**overrides) -> Dict[str, Any]:
        ...
        data = {
            ...
            "agent_type": random.choice(["frontend", "backend", "fullstack", "devops", "qa"]),
            "status": random.choice(["active", "idle", "busy", "offline"]),
            "description": f"Test agent {TestDataGenerator.random_string(10)}",
            "model": random.choice(["claude-3-sonnet", "claude-3-haiku", "claude-3-opus"]),
            "tools": random.sample(["read", "write", "edit", "bash", "grep"], 3),
            "capabilities": random.sample(["python", "javascript", "typescript", "docker"], 2),
            "configuration": {
    ...
    def workflow_data(**overrides) -> Dict[str, Any]:
        ...
        for i in range(num_steps):
            step = {
                "name": f"step_{i+1}",
                "type": "task",
                "dependencies": [f"step_{j+1}" for j in range(i) if random.choice([True, False])]
            }
            steps.append(step)
...
class DatabaseTestHelper:
    ...
    async def verify_foreign_key_constraint(self, model_class, field_name: str, invalid_id: str):
...
def assert_response_schema(response_data: Dict, required_fields: List[str], optional_fields: List[str] = None):
