...
class ReportGenerator:
    ...
    def __init__(self, analytics_service: AnalyticsService, output_dir: str = "reports"):
    ...
    async def generate_report(
        ...
        try:
            ...
            self.logger.info(f"Successfully generated {config.report_type.value} report")
            return result
    ...
    async def _prepare_report_data(
        ...
        report_data = {
            "title": config.custom_title or f"{config.report_type.value.title()} Report",
            ...
            if config.include_recommendations:
                if "optimization" in analytics_data:
                    ...
                    if "recommendations" in optimization:
                        report_data["recommendations"] = optimization["recommendations"][:5]  # Top 5
        ...
        try:
            ...
            if "performance_analysis" in analytics_data:
                ...
                if "system_metrics" in performance:
                    ...
                    efficiency_chart = await self._create_efficiency_chart(system_metrics)
                ...
                if "detailed_agent_metrics" in performance:
                    ...
                    agent_chart = await self._create_agent_performance_chart(agent_metrics)
    ...
    async def _create_efficiency_chart(self, system_metrics: Dict[str, Any]) -> str:
        ...
        try:
            ...
            categories = ['System Efficiency', 'Success Rate', 'Uptime', 'Resource Utilization']
            values = [
            ...
            bars = ax.bar(categories, values, color=['#2E8B57', '#4682B4', '#32CD32', '#FF6347'])
    ...
    async def _create_agent_performance_chart(self, agent_metrics: List[Dict[str, Any]]) -> str:
        ...
        try:
            ...
            agent_names = [agent.get('name', 'Unknown')[:15] for agent in agent_metrics[:10]]  # Top 10
            efficiency_scores = [agent.get('efficiency_score', 0) for agent in agent_metrics[:10]]
        ...
        try:
            ...
            for i, trend in enumerate(trends[:3]):  # Show top 3 trends
                ...
                if direction == 'increasing':
                    values = np.linspace(50, 80, len(time_points)) + np.random.normal(0, 3, len(time_points))
                elif direction == 'decreasing':
                    values = np.linspace(80, 50, len(time_points)) + np.random.normal(0, 3, len(time_points))
                else:
                    values = np.full(len(time_points), 65) + np.random.normal(0, 5, len(time_points))
                
                ax.plot(time_points, values, label=metric_name, marker='o', markersize=2, alpha=0.8)
    ...
    <div class="container">
        ...
        """
            ...
            for insight in report_data["insights"][:10]:  # Top 10
                ...
                priority_class = "critical" if priority == "critical" else "high-priority" if priority == "high" else ""
        ...
        # Add recommendations
        if config.include_recommendations and "recommendations" in report_data and report_data["recommendations"]:
            html += """
        <div class="section">
