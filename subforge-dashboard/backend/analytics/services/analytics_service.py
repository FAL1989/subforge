...
class AnalyticsService:
    ...
    def __init__(self, storage_path: Optional[str] = None):
        self.storage_path = Path(storage_path) if storage_path else Path("analytics_data")
    ...
    async def run_comprehensive_analysis(
        ...
        """
        try:
            self.logger.info(f"Starting comprehensive analysis (time_range: {time_range_hours}h)")
            ...
            analysis_tasks.append(
                self._run_with_error_handling(
                    ...
                    self.performance_analyzer.analyze(db, time_range_hours=time_range_hours)
                )
            )
            ...
            analysis_tasks.append(
                self._run_with_error_handling(
                    "insight_generation",
                    self.insight_generator.analyze(db, time_range_hours=time_range_hours)
                )
            )
            ...
            if include_predictions and self.config["enable_predictive_models"]:
                analysis_tasks.append(
                    self._run_with_error_handling(
                        ...
                        self.task_duration_predictor.analyze(db, time_range_hours=time_range_hours)
                    )
                )
                ...
                analysis_tasks.append(
                    self._run_with_error_handling(
                        ...
                        self.system_load_predictor.analyze(db, time_range_hours=time_range_hours)
                    )
                )
            ...
            if include_optimization:
                analysis_tasks.append(
                    self._run_with_error_handling(
                        ...
                        self.optimization_engine.analyze(db, time_range_hours=time_range_hours)
                    )
                )
            ...
            if include_predictions and len(results) > 4:
                comprehensive_report["predictions"] = {
                    ...
                    "system_load": results[5].get("result", {}) if len(results) > 5 else {}
                }
            ...
            if include_optimization:
                ...
                if len(results) > optimization_index:
                    comprehensive_report["optimization"] = results[optimization_index].get("result", {})
            ...
            self.logger.info(f"✅ Comprehensive analysis completed in {(datetime.utcnow() - start_time).total_seconds():.2f}s")
    ...
    async def run_predictive_analysis(
        ...
        try:
            ...
            if prediction_type in ["task_duration", "both"]:
                results["task_duration"] = await self.task_duration_predictor.analyze(db)
    ...
    async def get_real_time_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for metric in metrics_to_fetch:
                stats = self.data_aggregator.get_current_metric_stats(metric, period="5min")
                if stats:
    ...
    async def add_metric_data_point(
        ...
        try:
            await self.data_aggregator.add_real_time_metric(metric_name, value, timestamp)
        except Exception as e:
    ...
    async def generate_weekly_report(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "improvement_opportunities": self._extract_improvement_opportunities(analysis)
            }
    ...
    def get_service_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "components": {
                ...
                "predictive_models": "active" if self.config["enable_predictive_models"] else "disabled",
            ...
            "last_analysis_runs": {
                key: timestamp.isoformat() if isinstance(timestamp, datetime) else timestamp
    ...
    async def _run_with_error_handling(self, component_name: str, coroutine) -> Dict[str, Any]:
    ...
    def _extract_key_highlights(self, analysis_data: Dict[str, Any]) -> List[str]:
        ...
        try:
            ...
            if insights:
                insight_data = insights.get("insights", [])
                if insight_data:
                    critical_count = len([i for i in insight_data if i.get("priority") == "critical"])
                    if critical_count > 0:
                        highlights.append(f"{critical_count} critical issues identified")
            
            # Trend highlights
            trends = analysis_data.get("trend_analysis", {})
            if trends:
                trend_data = trends.get("trends", [])
                improving_trends = len([t for t in trend_data if t.get("direction") == "increasing"])
                if improving_trends > 0:
                    highlights.append(f"{improving_trends} metrics showing positive trends")
    ...
    def _extract_improvement_opportunities(self, analysis_data: Dict[str, Any]) -> List[str]:
    ...
    async def _auto_analysis_loop(self):
        ...
        while True:
            try:
                ...
                self.logger.info("Auto-analysis interval reached (database session needed)")
            except asyncio.CancelledError:
                break
            except Exception as e:
