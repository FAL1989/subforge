...
class BaseAnalyzer(ABC):
    ...
    def _validate_sample_size(
        self, data_count: int, operation: str = "analysis"
    ) -> bool:
    ...
    def _calculate_confidence_score(
        self, data_points: int, variance: float, time_span_hours: float
    ) -> float:
        """
        ...
        size_confidence = min(
            data_points / 100.0, 1.0
        )  # Up to 100 points gives full confidence
        ...
        time_confidence = min(
            time_span_hours / 168.0, 1.0
        )  # 1 week = full time confidence
    ...
    def _generate_insights(
        ...
        if thresholds:
            for threshold_name, threshold_value in thresholds.items():
                if (
                    threshold_name == "critical_high"
                    and current_value > threshold_value
                ):
                    insights.append(
                        f"{metric_name} exceeds critical threshold ({threshold_value})"
                    )
                elif (
                    threshold_name == "critical_low" and current_value < threshold_value
                ):
                    insights.append(
                        f"{metric_name} below critical threshold ({threshold_value})"
                    )
                elif (
                    threshold_name == "warning_high" and current_value > threshold_value
                ):
                    insights.append(
                        f"{metric_name} approaching high threshold ({threshold_value})"
                    )
                elif (
                    threshold_name == "warning_low" and current_value < threshold_value
                ):
                    insights.append(
                        f"{metric_name} approaching low threshold ({threshold_value})"
                    )
