...
class PerformanceAnalyzer(BaseAnalyzer):
    ...
    async def analyze_agent_performance(
        ...
        try:
            ...
            self.logger.info(
                f"Analyzed performance for {len(performance_metrics)} agents"
            )
    ...
    async def _analyze_single_agent(
        self, db: AsyncSession, agent: Agent, start_time: datetime
    ) -> AgentPerformanceMetrics:
        """Analyze performance metrics for a single agent"""

        # Get agent tasks in time range
        task_query = select(Task).where(
            and_(Task.assigned_agent_id == agent.id, Task.created_at >= start_time)
        )
        ...
        success_rate = (
            (len(completed_tasks) / total_attempted * 100) if total_attempted > 0 else 0
        )
        ...
        reliability_score = self._calculate_reliability_score(
            success_rate, agent.uptime_percentage
        )
    ...
    def _calculate_productivity_score(
        self, agent: Agent, completed_tasks: List[Task]
    ) -> float:
        ...
        # Quality bonus
        quality_scores = [
            t.quality_score for t in completed_tasks if t.quality_score is not None
        ]
        if quality_scores:
    ...
    def _calculate_reliability_score(
        self, success_rate: float, uptime_percentage: float
    ) -> float:
    ...
    async def _analyze_performance_trend(
        self, db: AsyncSession, agent_id: str, start_time: datetime
    ) -> Dict[str, Any]:
        ...
        try:
            ...
            interval_hours = (
                (datetime.utcnow() - start_time).total_seconds() / 3600 / intervals
            )
    ...
    async def analyze_system_performance(
        self, db: AsyncSession, time_range_hours: int = 24
        ...
        try:
            ...
            avg_task_completion_time = (
                np.mean(completion_times) if completion_times else 0
            )
            ...
            if latest_metrics:
                ...
                cpu_utilization = memory_utilization = system_load = (
                    network_utilization
                ) = 0
            ...
            optimization_opportunities = (
                await self._identify_optimization_opportunities(db, agents, tasks)
            )
    ...
    async def _identify_bottlenecks(
        self, db: AsyncSession, agents: List[Agent], tasks: List[Task]
    ) -> List[Dict[str, Any]]:
        ...
        overloaded_agents = [
            ...
            for a in agents
            if a.status == "active"
            and len([t for t in tasks if t.assigned_agent_id == a.id]) > 10
        ]
    ...
    async def _identify_optimization_opportunities(
        ...
        for agent_type, count in agent_types.items():
            type_tasks = [
                ...
                for t in tasks
                if any(
                    a.agent_type == agent_type and a.id == t.assigned_agent_id
                    for a in agents
                )
            ]
        ...
        try:
            ...
            if agent_metrics:
                top_performers = sorted(
                    agent_metrics, key=lambda x: x.efficiency_score, reverse=True
                )[:5]
                bottom_performers = sorted(
                    agent_metrics, key=lambda x: x.efficiency_score
                )[:3]
    ...
    def _generate_performance_insights(
        ...
        insights = {"key_findings": [], "recommendations": [], "alerts": []}
        ...
        improving_agents = [
            m for m in agent_metrics if m.performance_trend == "improving"
        ]
        declining_agents = [
            m for m in agent_metrics if m.performance_trend == "declining"
        ]

        if len(improving_agents) > len(declining_agents):
            insights["key_findings"].append(
                f"{len(improving_agents)} agents showing performance improvements"
            )
        elif len(declining_agents) > len(improving_agents):
            insights["key_findings"].append(
                f"{len(declining_agents)} agents showing performance decline"
            )
            insights["recommendations"].append(
                "Review and support underperforming agents"
            )
        ...
        [m.success_rate for m in agent_metrics]
        low_success_agents = [m for m in agent_metrics if m.success_rate < 70]

        if low_success_agents:
            insights["alerts"].append(
                f"{len(low_success_agents)} agents have success rates below 70%"
            )
            insights["recommendations"].append(
                "Investigate and improve low-performing agents"
            )
