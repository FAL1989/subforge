...
class TaskDurationPredictor(BaseAnalyzer):
    ...
    async def train_model(
        self, db: AsyncSession, retrain: bool = False
    ) -> ModelPerformance:
        """
        ...
        try:
            ...
            for name, model in models.items():
                ...
                model_performances[name] = {"mse": mse, "r2": r2, "rmse": np.sqrt(mse)}
                trained_models[name] = model

            # Select best performing model
            best_model_name = max(
                model_performances.keys(), key=lambda k: model_performances[k]["r2"]
            )
            ...
            self.logger.info(
                f"Model trained successfully. Best model: {best_model_name}, "
    ...
    def _extract_features_and_target(
        ...
        df["task_priority_encoded"] = (
            df["task_priority"].map(priority_mapping).fillna(2)
        )
    ...
    def _calculate_prediction_confidence(
        self, X_scaled: np.ndarray, prediction: float
    ) -> float:
                ...
                predictions.append(
                    {
                        ...
                        "estimated_completion": (
                            datetime.utcnow()
                            + timedelta(minutes=prediction_result.predicted_value)
                        ).isoformat(),
                    }
                )
            except Exception as e:
                self.logger.warning(
                    f"Failed to predict duration for task {task.id}: {e}"
                )
...
class SystemLoadPredictor(BaseAnalyzer):
    ...
    def _prepare_load_training_data(
    ...
    async def predict_system_load(
        ...
        for hour in range(1, hours_ahead + 1):
            ...
            future_features = [
                ...
                current_metrics.get("total_tasks", 10)
                + (hour * 2),  # Assume 2 tasks per hour
    ...
    async def _get_current_system_state(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        return {
            ...
            "memory_usage": (
                latest_metrics.memory_usage_percentage if latest_metrics else 40
            ),
            "api_requests": (
                latest_metrics.api_requests_per_minute if latest_metrics else 10
            ),
        }
    ...
    async def generate_capacity_recommendations(
        ...
        high_load_periods = [
            p for p in load_predictions if p["predicted_load_percentage"] > 80
        ]
        ...
        low_load_periods = [
            p for p in load_predictions if p["predicted_load_percentage"] < 30
        ]
