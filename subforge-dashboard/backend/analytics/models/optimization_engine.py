...
class OptimizationEngine(BaseAnalyzer):
    ...
    async def analyze_system_performance(self, db: AsyncSession) -> SystemAnalysis:
        ...
        try:
            ...
            efficiency_score = await self._calculate_system_efficiency(agents, tasks, workflows)
            
            # Identify bottlenecks
            bottlenecks = await self._identify_system_bottlenecks(db, agents, tasks, workflows)
            
            # Find underutilized resources
            underutilized_resources = await self._identify_underutilized_resources(agents, tasks)
            
            # Analyze performance trends
            performance_trends = await self._analyze_performance_trends(db, system_metrics)
    ...
    async def generate_recommendations(
        ...
        recommendations.extend(await self._generate_workflow_recommendations(db, analysis))
        
        # Cost optimization recommendations
        recommendations.extend(await self._generate_cost_recommendations(analysis))
        
        # Reliability improvements
        recommendations.extend(await self._generate_reliability_recommendations(db, analysis))
    ...
    async def _generate_performance_recommendations(
        ...
        if analysis.efficiency_score < 70:
            recommendations.append(OptimizationRecommendation(
                ...
                affected_components=["task_scheduler", "agent_manager", "performance_monitor"],
        ...
        for bottleneck in analysis.bottlenecks:
            if bottleneck.get("severity") == "high":
                recommendations.append(OptimizationRecommendation(
                    ...
                    implementation_steps=self._get_bottleneck_resolution_steps(bottleneck),
                    evidence=bottleneck,
    ...
    async def _generate_workflow_recommendations(
        ...
        long_workflows = [w for w in workflows if getattr(w, 'avg_duration_hours', 0) > 8]
    ...
    async def _generate_cost_recommendations(
        ...
        # Idle resource costs
        idle_resources = [r for r in analysis.underutilized_resources 
        ...
        if idle_resources:
            ...
            recommendations.append(OptimizationRecommendation(
                ...
                evidence={"idle_resource_count": len(idle_resources), "estimated_monthly_cost": total_idle_cost},
        ...
        if analysis.efficiency_score < 80:
            potential_savings = (80 - analysis.efficiency_score) * 10  # $10 per efficiency point
            recommendations.append(OptimizationRecommendation(
                ...
                evidence={"current_efficiency": analysis.efficiency_score, "potential_savings": potential_savings},
    ...
    def _prioritize_recommendations(
        ...
        def priority_score(rec: OptimizationRecommendation) -> float:
            ...
            complexity_penalty = complexity_penalties.get(rec.implementation_complexity, -10)
    ...
    def _generate_implementation_roadmap(
        ...
        medium_recs = [r for r in recommendations if r.priority == OptimizationPriority.MEDIUM]
        low_recs = [r for r in recommendations if r.priority == OptimizationPriority.LOW]
        ...
        roadmap["resource_requirements"] = {
            ...
            "recommended_team_size": max(1, min(4, total_effort // 80))  # 2 weeks per person max
        }
    ...
    def _generate_summary(
        ...
        total_impact = sum(r.estimated_impact_percentage for r in recommendations[:5])  # Top 5
        ...
        return {
            "system_health": {
                ...
                "status": "good" if analysis.efficiency_score > 80 else 
                         "needs_attention" if analysis.efficiency_score > 60 else "critical"
            },
            ...
            "key_areas": {
                ...
                "primary_focus_area": max(by_category.keys(), key=lambda k: by_category[k]) if by_category else "none"
            },
    ...
    def _get_bottleneck_resolution_steps(self, bottleneck: Dict[str, Any]) -> List[str]:
        ...
        return resolution_steps.get(bottleneck_type, [
    ...
    async def _analyze_performance_trends(
        ...
        efficiency_values = [m.overall_success_rate for m in sorted_metrics[-10:]]  # Last 10 points
    ...
    def _analyze_resource_allocation(
        ...
        if agent_task_counts:
            ...
            imbalance_score = min(variance / (avg_tasks + 1), 1.0)  # Normalized imbalance
        else:
            imbalance_score = 0
