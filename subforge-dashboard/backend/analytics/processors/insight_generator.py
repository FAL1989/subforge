...
class InsightGenerator(BaseAnalyzer):
    ...
    async def analyze(self, db: AsyncSession, **kwargs) -> Dict[str, Any]:
        ...
        summary = await self._generate_insight_summary(
            prioritized_insights, intelligence_data
        )
    ...
    async def _collect_system_intelligence(self, db: AsyncSession) -> Dict[str, Any]:
        ...
        try:
            ...
            metrics_query = (
                ...
                .where(SystemMetrics.recorded_at >= last_day)
    ...
    async def _analyze_agent_intelligence(self, agents: List[Agent]) -> Dict[str, Any]:
        ...
        response_times = [
            a.avg_response_time for a in agents if a.avg_response_time > 0
        ]
        ...
        workload_balance = (
            np.std(task_counts) / np.mean(task_counts)
    ...
    async def _analyze_task_intelligence(self, tasks: List[Task]) -> Dict[str, Any]:
        ...
        recent_tasks = [
            t
            for t in tasks
            if t.created_at and (now - t.created_at).total_seconds() < 3600
        ]  # Last hour
        ...
        agent_task_counts = Counter(
            [str(t.assigned_agent_id) for t in tasks if t.assigned_agent_id]
        )

        # Identify bottlenecks
        bottlenecks = []
        if len(recent_tasks) < len(
            [t for t in tasks if (now - t.created_at).total_seconds() < 7200]
        ):  # Slowdown
    ...
    async def _analyze_system_intelligence(
        ...
        if len(metrics) >= 5:
            ...
            cpu_trend = self._calculate_simple_trend(
                [m.cpu_usage_percentage for m in recent_metrics]
            )
            memory_trend = self._calculate_simple_trend(
                [m.memory_usage_percentage for m in recent_metrics]
            )
            load_trend = self._calculate_simple_trend(
                [m.system_load_percentage for m in recent_metrics]
            )
        else:
    ...
    async def _analyze_workflow_intelligence(
        ...
        success_rate = (
            (len(completed_workflows) / total_workflows * 100)
            ...
            insights.append(
                self._create_insight(
                    ...
                    evidence={
                        ...
                        "health_issues": system_data.get("health_assessment", {}).get(
                            "health_issues", []
                        ),
                    },
        ...
        if high_priority_backlog > 10:
            insights.append(
                self._create_insight(
                    ...
                    priority=InsightPriority.HIGH,
                    evidence={
                        "backlog_count": high_priority_backlog,
                        "task_distribution": task_data.get("priority_distribution", {}),
                    },
            ...
            insights.append(
                self._create_insight(
                    ...
                    evidence={
                        ...
                        "idle_agents": agent_data.get("status_distribution", {}).get(
                            "idle", 0
                        ),
                    },
    ...
    async def _generate_ai_insights(
        ...
        avg_success_rate = agent_data.get("performance_metrics", {}).get(
    ...
    def _create_insight(
        ...
        insight_id = (
            f"insight_{int(datetime.utcnow().timestamp())}_{hash(title) % 10000}"
        )

        # Determine business value and risk level
        business_value = self._assess_business_value(
            impact_description, estimated_impact_percentage
        )
    ...
    def _assess_business_value(
        self, impact_description: str, impact_percentage: float
    ) -> str:
    ...
    def _assess_risk_level(
        self, priority: InsightPriority, impact_percentage: float
    ) -> str:
    ...
    async def _generate_insight_summary(
        self, insights: List[Insight], intelligence_data: Dict[str, Any]
    ) -> Dict[str, Any]:
        ...
        # Key insights
        critical_insights = [
            i for i in insights if i.priority == InsightPriority.CRITICAL
        ]
        high_impact_insights = [
            i for i in insights if i.estimated_impact_percentage > 20
        ]

        # Generate action plan
        immediate_actions = [
            i for i in insights if i.action_type == ActionType.IMMEDIATE
        ]
        ...
        return {
            ...
            "impact_assessment": {
                ...
                "roi_estimate": (
                    f"{total_impact / (total_effort / 40):.1f}% per week"
                    if total_effort > 0
                    else "N/A"
                ),
            },
    ...
    def _generate_executive_summary(
        ...
        if health_score < 70:
            summary.append(
                f"System health is below optimal at {health_score}/100 - immediate attention required"
            )
        elif health_score < 90:
            summary.append(
                f"System health is good at {health_score}/100 with room for improvement"
            )
        else:
        ...
        if critical_count > 0:
            summary.append(
                f"{critical_count} critical issues identified requiring immediate action"
            )

        # Opportunity insights
        efficiency_insights = len(
            [i for i in insights if i.insight_type == InsightType.EFFICIENCY]
        )
        if efficiency_insights > 0:
            summary.append(
                f"{efficiency_insights} efficiency improvement opportunities identified"
            )

        # Performance insights
        performance_insights = len(
            [i for i in insights if i.insight_type == InsightType.PERFORMANCE]
        )
        if performance_insights > 0:
            summary.append(
                f"{performance_insights} performance optimization recommendations available"
            )
