...
class TrendAnalyzer(BaseAnalyzer):
    ...
    async def _collect_time_series_data(
                ...
                for metric in system_metrics:
                    if hasattr(metric, attribute) and metric.recorded_at:
                        ...
                        if value is not None:
                            data.append(
                                {"timestamp": metric.recorded_at, "value": float(value)}
                            )
        ...
        tasks_query = (
            select(Task)
            .where(and_(Task.created_at >= start_time, Task.created_at <= end_time))
        ...
        if tasks:
            ...
            while current_hour <= end_time:
                ...
                hour_tasks = [
                    t for t in tasks if current_hour <= t.created_at < next_hour
                ]
                ...
                if hour_tasks:
                    ...
                    task_data.append(
                        {"timestamp": current_hour, "value": completion_rate}
                    )
    ...
    async def _analyze_single_trend(
        ...
        try:
            ...
            df = df.copy()
            df["time_numeric"] = (
                pd.to_numeric(df["timestamp"]) / 10**9
            )  # Convert to seconds
    ...
    async def _analyze_single_seasonality(
        self, metric_name: str, df: pd.DataFrame
    ) -> SeasonalityResult:
        ...
        try:
            ...
            daily_cv = (
                hourly_std.mean() / hourly_means.mean()
            ...
            if len(df) >= 14 * 24:  # At least 2 weeks of hourly data
                ...
                weekly_cv = (
                    weekly_means.std() / weekly_means.mean()
                    if weekly_means.mean() != 0
                    else 0
                )
            else:
                weekly_means = None
            ...
            for lag in lags_to_test:
                if len(df) > lag * 2:
                    ...
                    autocorr_scores.append(
                        abs(autocorr) if not pd.isna(autocorr) else 0
                    )
                else:
            ...
            if daily_cv > 0.1 and autocorr_scores[0] > 0.3:
                ...
                weekly_cv > 0.1
                and len(autocorr_scores) > 1
                and autocorr_scores[1] > 0.3
            ):
                has_seasonality = True
            ...
            return SeasonalityResult(
                ...
                period_hours=(
                    ...
                    else 168.0 if seasonality_type == SeasonalityType.WEEKLY else None
                ),
                strength=strength,
    ...
    async def _detect_metric_patterns(
        ...
        try:
            ...
            for idx in anomaly_indices:
                patterns.append(
                    PatternDetection(
                        ...
                        confidence=min(
                            (z_scores[idx] - anomaly_threshold) / anomaly_threshold, 1.0
                        ),
            ...
            peaks, properties = find_peaks(
                values, height=np.mean(values) + 2 * np.std(values), distance=5
            )
            ...
            if len(values) >= 30:
                ...
                significant_changes = np.where(
                    np.abs(slope_changes) > change_threshold
                )[0]
                    ...
                    if actual_idx < len(timestamps) - 1:
                        patterns.append(
                            PatternDetection(
                                ...
                                end_time=timestamps[
                                    min(actual_idx + window_size, len(timestamps) - 1)
                                ],
    ...
    async def _generate_trend_summary(
        ...
        trend_directions = {}
        for trend in trend_results:
            trend_directions[trend.direction.value] = (
                trend_directions.get(trend.direction.value, 0) + 1
            )
        ...
        # Strong trends
        strong_trends = [
            t for t in trend_results if t.strength > 0.7 and t.confidence > 0.8
        ]
        if strong_trends:
        ...
        if seasonal_metrics:
            key_findings.append(
                f"Found {len(seasonal_metrics)} metrics with seasonal patterns"
            )
        ...
        if len(anomalies) > 5:
            key_findings.append(
                f"High anomaly count detected: {len(anomalies)} anomalies"
            )
        ...
        return {
            ...
            "analysis_quality": {
                "avg_confidence": (
                    np.mean([t.confidence for t in trend_results])
                    if trend_results
                    else 0
                ),
                "avg_data_points": (
                    np.mean([t.data_points for t in trend_results])
                    if trend_results
                    else 0
                ),
    ...
    def _generate_trend_recommendations(
        ...
        declining_trends = [
            ...
            if t.direction == TrendDirection.DECREASING and t.confidence > 0.6
        ]
        if declining_trends:
            recommendations.append(
                f"Monitor declining trends in: {', '.join([t.metric_name for t in declining_trends])}"
            )

        # Volatile metrics
        volatile_metrics = [
            t for t in trend_results if t.direction == TrendDirection.VOLATILE
        ]
        if volatile_metrics:
            recommendations.append(
                f"Investigate high volatility in: {', '.join([t.metric_name for t in volatile_metrics])}"
            )

        # Seasonal patterns
        daily_seasonal = [
            s
            for s in seasonality_results
            if s.seasonality_type == SeasonalityType.DAILY
        ]
        if daily_seasonal:
            recommendations.append(
                f"Leverage daily patterns for optimization in: {', '.join([s.metric_name for s in daily_seasonal])}"
            )

        # High anomaly count
        anomaly_count = len(
            [p for p in pattern_detections if p.pattern_type == "anomaly"]
        )
        if anomaly_count > 10:
            recommendations.append(
                "High anomaly count suggests system instability - investigate root causes"
            )

        # Trend changes
        trend_changes = [
            p for p in pattern_detections if p.pattern_type == "trend_change"
        ]
        if trend_changes:
            recommendations.append(
                f"Recent trend changes detected - validate system changes or external factors"
            )
