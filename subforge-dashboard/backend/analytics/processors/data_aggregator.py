...
class DataAggregator(BaseAnalyzer):
    ...
    def _initialize_metric_windows(self):
        ...
        window_configs = {"1min": 1, "5min": 5, "15min": 15, "1hour": 60, "1day": 1440}
    ...
    async def analyze(self, db: AsyncSession, **kwargs) -> Dict[str, Any]:
        ...
        return {
            ...
            "aggregation_metadata": {
                ...
                "total_windows": sum(
                    len(windows) for windows in self.metric_windows.values()
                ),
                "buffer_sizes": {
                    key: len(buffer) for key, buffer in self.real_time_buffers.items()
                },
            },
        }
    ...
    async def _aggregate_for_period(
        ...
        period_tasks = [
            t for t in tasks if t.created_at and t.created_at >= period_start
        ]
        ...
        avg_agent_success_rate = (
            np.mean(agent_success_rates) if agent_success_rates else 0
        )
        avg_agent_response_time = (
            np.mean(agent_response_times) if agent_response_times else 0
        )
        ...
        error_rate = (
            (len(failed_tasks) / total_attempted * 100) if total_attempted > 0 else 0
        )
    ...
    async def _update_metric_windows(
        self, aggregations: Dict[str, Any], timestamp: datetime
    ):
        ...
        for period, metrics in aggregations.items():
            if period in self.metric_windows:
                ...
                if "task_completion_time" in windows:
                    windows["task_completion_time"].add_point(
                        metrics.avg_completion_time, timestamp
                    )
                if "agent_response_time" in windows:
                    windows["agent_response_time"].add_point(
                        metrics.agent_response_time, timestamp
                    )
                if "tasks_per_minute" in windows:
                    windows["tasks_per_minute"].add_point(
                        metrics.throughput_tasks_per_hour / 60, timestamp
                    )
                if "error_rate" in windows:
                    windows["error_rate"].add_point(
                        metrics.error_rate_percentage, timestamp
                    )
                if "success_rate" in windows:
                    windows["success_rate"].add_point(
                        metrics.agent_success_rate, timestamp
                    )
    ...
    async def aggregate_historical_metrics(
        ...
        try:
            ...
            # Get historical data
            historical_metrics = await self._get_historical_system_metrics(
                db, start_time, end_time
            )
            historical_tasks = await self._get_historical_tasks(
                db, start_time, end_time
            )

            # Generate daily, weekly aggregations
            daily_aggregates = await self._aggregate_by_day(
                historical_metrics, historical_tasks
            )
            weekly_aggregates = await self._aggregate_by_week(
                historical_metrics, historical_tasks
            )
    ...
    async def _aggregate_by_day(
        self, metrics: List[SystemMetrics], tasks: List[Task]
    ) -> List[Dict[str, Any]]:
        ...
        for day, data in sorted(daily_groups.items()):
            ...
            if day_metrics:
                ...
                avg_success_rate = np.mean(
                    [m.overall_success_rate for m in day_metrics]
                )
            else:
    ...
    async def _aggregate_by_week(
        ...
        for week, data in sorted(weekly_groups.items()):
            ...
            if week_metrics:
                ...
                avg_success_rate = np.mean(
                    [m.overall_success_rate for m in week_metrics]
                )
    ...
    def get_current_metric_stats(
        self, metric_name: str, period: str = "5min"
    ) -> Dict[str, Any]:
    ...
    def get_metric_history(
        ...
        return [
            (timestamp, value)
            for timestamp, value in self.real_time_buffers[metric_name]
    ...
    async def _get_latest_system_metrics(
