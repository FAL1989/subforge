...
class ConnectionManager:
    ...
    def get_connection_info(self) -> List[Dict[str, Any]]:
        ...
        for connection in self.active_connections:
            ...
            connection_info = {
                ...
                "duration_seconds": (current_time - metadata.get("connected_at", current_time)).total_seconds(),
                "message_count": metadata.get("message_count", 0),
                "last_activity": metadata.get("last_activity", current_time).isoformat(),
    ...
    async def cleanup_stale_connections(self, max_inactive_minutes: int = 30):
        ...
        for connection in stale_connections:
            logger.info(f"Cleaning up stale WebSocket connection (inactive for {max_inactive_minutes}+ minutes)")
            try:
