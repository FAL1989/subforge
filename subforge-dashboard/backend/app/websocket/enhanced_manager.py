...
class EnhancedConnectionManager:
    ...
    def __init__(self):
        ...
        # Room management
        self.rooms: Dict[str, Set[str]] = defaultdict(
            set
    ...
    async def disconnect(self, connection_id: str):
        ...
        try:
            ...
            logger.info(
                f"Connection cleaned up: {connection_id} (Total: {len(self.active_connections)})"
            )
    ...
    async def authenticate_connection(
        self, connection_id: str, user_id: str, permissions: Set[str] = None
    ) -> bool:
    ...
    async def join_room(self, connection_id: str, room_name: str) -> bool:
        ...
        try:
            ...
            logger.error(
                f"Error joining room {room_name} for connection {connection_id}: {e}"
            )
            return False
        ...
        try:
            ...
            await self.send_to_connection(
                ...
                WebSocketMessage(
                    ...
                    room=room_name,
                    data={"room": room_name, "status": "left"},
                ),
            )
            ...
            logger.error(
                f"Error leaving room {room_name} for connection {connection_id}: {e}"
            )
            return False
    ...
    async def broadcast_to_all(
        self, message: WebSocketMessage, exclude_connection: Optional[str] = None
    ) -> int:
    ...
    async def add_to_event_history(
        ...
        if self.redis_client:
            try:
                await self.redis_client.lpush("websocket:event_history", entry.json())
                await self.redis_client.ltrim(
                    "websocket:event_history", 0, self.max_history_size - 1
                )
            except Exception as e:
    ...
    def get_connection_stats(self) -> Dict[str, Any]:
        ...
        room_stats = {
            room_name: len(connections) for room_name, connections in self.rooms.items()
        }
    ...
    async def _periodic_cleanup(self):
        ...
        while True:
            try:
                ...
                # Clean up empty rooms
                empty_rooms = [
                    name for name, connections in self.rooms.items() if not connections
                ]
                ...
                if stale_connections or empty_rooms:
                    logger.info(
                        f"Cleanup completed: {len(stale_connections)} stale connections, {len(empty_rooms)} empty rooms"
                    )
