...
class SocketIOManager:
    ...
    def create_server(self) -> AsyncServer:
        ...
        logger.info("Socket.IO server created with CORS origins: %s", cors_allowed_origins)
        return self.sio
    ...
    def _register_event_handlers(self):
        ...
        async def disconnect(sid: str):
            ...
            try:
                ...
                logger.error(f"Error handling Socket.IO disconnection: {e}", exc_info=True)
        ...
        async def join_room(sid: str, data: Dict[str, Any]):
            ...
            try:
                ...
                if not room_name or room_name not in [room.value for room in SocketIORoom]:
                    await self.sio.emit(
                        SocketIOEvent.ERROR,
        ...
        async def agent_status_update(sid: str, data: Dict[str, Any]):
            ...
            try:
                ...
                logger.debug(f"Agent status update: {data['agent_id']} -> {data['status']}")
        ...
        async def system_metrics_update(sid: str, data: Dict[str, Any]):
            ...
            try:
                ...
                logger.error(f"Error handling system metrics update: {e}", exc_info=True)
    ...
    async def cleanup_disconnected_clients(self):
        ...
        try:
            ...
            if disconnected_clients:
                logger.info(f"Cleaned up {len(disconnected_clients)} disconnected Socket.IO clients")
...
async def emit_agent_status(agent_id: str, status: str, data: Optional[Dict[str, Any]] = None):
...
async def emit_task_update(task_id: str, status: str, data: Optional[Dict[str, Any]] = None):
...
async def emit_workflow_event(workflow_id: str, event_type: str, data: Optional[Dict[str, Any]] = None):
