...
class APIEnhancementService:
    ...
    async def get_api_statistics(
        ...
        try:
            ...
            stats = {
                ...
                "unique_ips": len(set(m.ip_address for m in all_metrics if m.ip_address)),
            ...
            if all_metrics:
                ...
                for metric in all_metrics:
                    ...
                    # Count by endpoints
                    stats["endpoints"][metric.endpoint] = stats["endpoints"].get(metric.endpoint, 0) + 1
                    
                    # Count by methods
                    stats["methods"][metric.method] = stats["methods"].get(metric.method, 0) + 1
    ...
    async def get_rate_limit_status(
        ...
        try:
            ...
            # Get current usage
            minute_key = f"{limit_key}:minute:{datetime.utcnow().strftime('%Y%m%d%H%M')}"
    ...
    async def _flush_metrics(self):
        ...
        try:
            ...
            for metric_data in metrics_data:
                ...
                metric_key = f"api_metric:{datetime.utcnow().isoformat()}:{id(metric_data)}"
                await redis_service.set(metric_key, metric_data, expire=604800)  # 7 days
                
                # Add to time-series list
                await redis_service.lpush("api_metrics_series", metric_data)
                await redis_service.ltrim("api_metrics_series", 0, 10000)  # Keep last 10k metrics
