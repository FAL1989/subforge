...
class PersistenceService:
    ...
    async def store_workflow_execution(
        ...
        try:
            ...
            workflow_history = WorkflowHistory(
                ...
                agent_performance_summary=workflow_data.get('agent_performance_summary', {}),
            ...
            self.logger.info(f"Stored workflow execution history: {workflow_history.id}")
    ...
    async def calculate_and_store_agent_performance(
        ...
        try:
            ...
            successful_workflows = len([w for w in agent_workflows if w.final_result == 'success'])
            failed_workflows = len([w for w in agent_workflows if w.final_result == 'failure'])
            ...
            avg_task_duration = (
                total_active_time / total_tasks 
                if total_tasks > 0 else None
            )
            
            avg_phase_duration = (
                total_active_time / total_phases 
                if total_phases > 0 else None
            )
            
            # Calculate quality metrics
            quality_scores = [p.quality_score for p in phases if p.quality_score is not None]
            avg_quality_score = sum(quality_scores) / len(quality_scores) if quality_scores else None
            
            performance_scores = [p.performance_score for p in phases if p.performance_score is not None]
            ...
            handoff_durations = [h.duration_seconds for h in handoffs if h.duration_seconds is not None]
            avg_handoff_duration = sum(handoff_durations) / len(handoff_durations) if handoff_durations else None
            ...
            utilization_percentage = min(100.0, (total_active_time / period_duration) * 100) if period_duration > 0 else 0.0
            ...
            self.logger.info(f"Calculated and stored agent performance metrics: {performance_metrics.id}")
    ...
    async def get_workflow_analytics_summary(
        ...
        try:
            ...
            successful_workflows = len([w for w in workflows if w.final_result == 'success'])
            success_rate = successful_workflows / total_workflows if total_workflows > 0 else 0.0
            
            durations = [w.duration_seconds for w in workflows if w.duration_seconds is not None]
            ...
            efficiency_scores = [w.efficiency_score for w in workflows if w.efficiency_score is not None]
    ...
    async def cleanup_old_records(
        ...
        try:
            ...
            self.logger.info(f"Cleaned up {workflow_count} old workflow records and {metrics_count} old metric records")
