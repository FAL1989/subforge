...
try:
    ...
    from ..models.subforge_models import FileSystemEvent as FSEvent
    from ..models.subforge_models import (
        PhaseResult,
...
class SubForgeFileHandler(FileSystemEventHandler):
    ...
    def _is_relevant_file(self, file_path: str) -> bool:
        ...
        return (
            file_path.name == "workflow_context.json"
            or file_path.suffix in [".json", ".md", ".log"]
            or file_path.parent.name
            in ["analysis", "generation", "deployment", "validation"]
        )
...
class SubForgeIntegrationService:
    ...
    def __init__(self, config: SubForgeIntegrationConfig = None):
        ...
        logger.info(
            f"SubForge integration initialized. Monitoring: {self.subforge_dir}"
        )
    ...
    async def _start_file_monitoring(self):
        ...
        try:
            ...
            self.observer.schedule(
                event_handler, str(self.subforge_dir), recursive=True
            )
            self.observer.start()
    ...
    async def handle_file_event(self, event_type: EventType, file_path: str):
        ...
        try:
            ...
            # Process the event
            if fs_event.is_workflow_file and event_type in [
                EventType.CREATED,
                EventType.MODIFIED,
            ]:
            ...
            for workflow_dir in self.subforge_dir.iterdir():
                if workflow_dir.is_dir() and workflow_dir.name.startswith("subforge_"):
                    ...
                    if workflow_context_file.exists():
                        ...
                        should_reload = (
                            workflow_id not in self.workflows
                            or workflow_context_file.stat().st_mtime
                        ...
                        if should_reload:
                            await self._process_workflow_file(
                                workflow_id, workflow_context_file
                            )
    ...
    async def _process_workflow_file(self, workflow_id: str, file_path: Path):
        ...
        try:
            ...
            logger.info(
                f"{'Loaded new' if was_new else 'Updated'} workflow: {workflow_id}"
            )

            # Persist workflow data if completed or failed
            if workflow_context.status in [
    ...
    def _determine_workflow_status(
        ...
        in_progress_phases = [
            r for r in phase_results.values() if r.status == "in_progress"
        ]
    ...
    def _enrich_workflow_context(
        self, workflow_context: WorkflowContext, raw_data: Dict[str, Any]
    ):
                    ...
                    if isinstance(task_data, dict):
                        task = Task(
                            ...
                            **{
                                ...
                                if k not in ["id", "title", "description", "status"]
                            },
                        )
                        tasks.append(task)
        ...
        for phase_name, phase_result in workflow_context.phase_results.items():
            if phase_result.status in ["completed", "in_progress"]:
                activity = AgentActivity(
                    ...
                    status=(
                        "completed" if phase_result.status == "completed" else "active"
                    ),
    ...
    async def _broadcast_file_event(self, event: FSEvent):
        ...
        try:
            await websocket_manager.broadcast_json(
                {"type": "subforge_file_event", "data": event.dict()}
            )
        except Exception as e:
            logger.error(f"Error broadcasting file event: {e}")

    async def _broadcast_workflow_update(
        self, workflow: WorkflowContext, is_new: bool = False
    ):
    ...
    async def _broadcast_workflow_list_update(self):
        ...
        try:
            workflow_summaries = [
                self._create_workflow_summary(w) for w in self.workflows.values()
            ]
            await websocket_manager.broadcast_json(
                {
    ...
    def get_workflows(self) -> List[WorkflowSummary]:
        ...
        return [
            self._create_workflow_summary(workflow)
    ...
    def get_workflow_metrics(self) -> WorkflowMetrics:
        ...
        recent_activities = sorted(
            all_activities, key=lambda x: x.timestamp, reverse=True
        )[:10]
        ...
        try:
            async for db_session in get_db():
                ...
                workflow_data = {
                    ...
                    "name": (
                        workflow.user_request[:255]
                        if workflow.user_request
                        else "Unknown Workflow"
                    ),
                    "type": "subforge_workflow",
                    "project_id": workflow.project_id,
                    "project_name": (
                        workflow.project_path.split("/")[-1]
                        if workflow.project_path
                        else workflow.project_id
                    ),
                    ...
                    "completed_at": (
                        workflow.updated_at
                        if workflow.status
                        in [WorkflowStatus.COMPLETED, WorkflowStatus.FAILED]
                        else None
                    ),
                    ...
                    "completed_tasks": (
                        len([t for t in workflow.tasks if t.status == "completed"])
                        if workflow.tasks
                        else 0
                    ),
                    "failed_tasks": (
                        len([t for t in workflow.tasks if t.status == "failed"])
                        if workflow.tasks
                        else 0
                    ),
                    ...
                    "agent_performance_summary": self._calculate_agent_performance_summary(
                        workflow
                    ),
                ...
                for phase_name, phase_result in workflow.phase_results.items():
                    phase_data = {
                        ...
                        "total_tasks": (
                            len([t for t in workflow.tasks if t.phase == phase_name])
                            if workflow.tasks
                            else 0
                        ),
                        "completed_tasks": (
                            len(
                                [
                                    ...
                                    if t.phase == phase_name and t.status == "completed"
                                ]
                            )
                            if workflow.tasks
                            else 0
                        ...
                        "failed_tasks": (
                            len(
                                [
                                    t
                                    for t in workflow.tasks
                                    if t.phase == phase_name and t.status == "failed"
                                ]
                            )
                            if workflow.tasks
                            else 0
                        ),
                        "dependencies": self._get_phase_dependencies(
                            phase_name, workflow
                        ),
    ...
    def _calculate_quality_score(self, workflow: WorkflowContext) -> Optional[float]:
        ...
        completed_phases = [
            p for p in workflow.phase_results.values() if p.status == "completed"
        ]
    ...
    def _calculate_efficiency_score(self, workflow: WorkflowContext) -> Optional[float]:
        ...
        duration_factor = 1.0
        if (
            hasattr(workflow, "expected_duration")
            ...
            if expected_duration > 0:
                duration_factor = min(
                    expected_duration / actual_duration, 2.0
    ...
    def _calculate_phase_quality_score(
        self, phase_result: PhaseResult
    ) -> Optional[float]:
    ...
    def _get_phase_dependencies(
        self, phase_name: str, workflow: WorkflowContext
    ) -> List[str]:
        ...
        for i in range(len(phases) - 1):
            ...
            if source_result and target_result:
                handoff = {
                    ...
                    "result": (
                        "success" if target_result.status == "completed" else "pending"
                    ),
    ...
    async def _store_agent_performance_metrics(
        self, session, workflow: WorkflowContext
    ):
