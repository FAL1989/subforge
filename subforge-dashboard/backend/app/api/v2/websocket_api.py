...
async def get_websocket_rooms():
    ...
    try:
        ...
        for room_name, connection_ids in enhanced_websocket_manager.rooms.items():
            ...
            room_info = RoomInfo(
                name=room_name,
                members_count=len(connection_ids),
                members=members
            )
...
@router.post(
    "/broadcast",
    summary="Broadcast message to WebSocket connections"
)
...
@router.post(
    ...
    summary="Leave a WebSocket room"
)
@api_enhancement_service.rate_limit(requests_per_minute=60, per_user=True)
...
@router.get(
    "/event-history",
    summary="Get WebSocket event history"
)
...
@router.get(
    "/statistics",
    summary="Get WebSocket system statistics"
)
@api_enhancement_service.cache_response(ttl=30)
@api_enhancement_service.rate_limit(requests_per_minute=60)
...
async def disconnect_websocket_connection(
    ...
    try:
        ...
        logger.info(f"Connection {connection_id} forcefully disconnected by {current_user.get('id')}")
