...
async def list_background_tasks(
    ...
    created_after: Optional[str] = Query(
        None, description="Filter tasks created after datetime"
    ),
    created_before: Optional[str] = Query(
        None, description="Filter tasks created before datetime"
    ),
...
@router.get(
    "/{task_id}", response_model=TaskResponse, summary="Get background task details"
)
...
async def create_background_task(
    request: TaskCreateRequest, current_user: dict = Depends(get_current_user)
):
...
@router.post("/{task_id}/cancel", summary="Cancel a background task")
...
@router.post("/{task_id}/retry", summary="Retry a failed background task")
@api_enhancement_service.rate_limit(requests_per_minute=30, per_user=True)
@require_auth
...
@router.post("/bulk-operation", summary="Perform bulk operations on multiple tasks")
@api_enhancement_service.rate_limit(requests_per_minute=10, per_user=True)
@require_auth
async def bulk_task_operation(
    request: BulkTaskOperation, current_user: dict = Depends(get_current_user)
):
...
@router.get("/queue-status", summary="Get task queue status information")
        ...
        queue_status = {
            "queues": {
                "default": {
                    ...
                    "active": len(
                        [t for t in all_tasks if t.status == TaskStatus.STARTED]
                    ),
                    "failed": len(
                        [t for t in all_tasks if t.status == TaskStatus.FAILURE]
                    ),
                }
            },
            "workers": {
...
@router.post("/cleanup", summary="Clean up old completed tasks")
@api_enhancement_service.rate_limit(requests_per_minute=5, per_user=True)
@require_auth
async def cleanup_old_tasks(
    older_than_days: int = Query(
        7, ge=1, le=365, description="Delete tasks older than N days"
    ),
...
@router.post(
    "/predefined/analyze-metrics", summary="Start system metrics analysis task"
)
@api_enhancement_service.rate_limit(requests_per_minute=10, per_user=True)
@require_auth
...
@router.post("/predefined/backup-data", summary="Start data backup task")
@api_enhancement_service.rate_limit(requests_per_minute=5, per_user=True)
@require_auth
