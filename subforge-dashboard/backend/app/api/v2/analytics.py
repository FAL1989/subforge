...
from ....analytics import (
    AnalyticsService, ReportGenerator, ReportFormat, ReportType, ReportConfig,
...
class AnalysisRequest(BaseModel):
    time_range_hours: int = Field(default=24, ge=1, le=720, description="Time range in hours (1-720)")
    ...
    agent_id: Optional[str] = Field(default=None, description="Specific agent ID for focused analysis")


class ReportGenerationRequest(BaseModel):
    report_type: str = Field(..., description="Type of report to generate")
    format: str = Field(default="json", description="Output format (json, csv, html, pdf)")
...
async def run_comprehensive_analysis(
    ...
    try:
        logger.info(f"Starting comprehensive analysis for {request.time_range_hours} hours")
...
async def run_predictive_analysis(
    prediction_type: str = Query(default="both", description="Prediction type: task_duration, system_load, or both"),
    db: AsyncSession = Depends(get_db),
    analytics: AnalyticsService = Depends(get_analytics_service)
) -> Dict[str, Any]:
...
async def run_trend_analysis(
    metrics: Optional[List[str]] = Query(default=None, description="Specific metrics to analyze"),
    db: AsyncSession = Depends(get_db),
    analytics: AnalyticsService = Depends(get_analytics_service)
) -> Dict[str, Any]:
    """Run trend analysis"""
...
async def get_metric_history(
    metric_name: str,
    minutes: int = Query(default=60, ge=1, le=1440, description="Time range in minutes"),
    analytics: AnalyticsService = Depends(get_analytics_service)
) -> Dict[str, Any]:
...
async def _generate_scheduled_reports_background(db: AsyncSession, report_gen: ReportGenerator):
    ...
    try:
        ...
        logger.info(f"Completed scheduled report generation: {len(reports)} report types")
    except Exception as e:
