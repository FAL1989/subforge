...
async def get_workflows(
    status: Optional[str] = Query(
        None, regex="^(active|inactive|paused|completed|failed|cancelled)$"
    ),
...
async def create_workflow(
    ...
    background_tasks.add_task(
        ...
        {"type": "workflow_created", "data": workflow.to_dict()},
    )
...
async def update_workflow(
    ...
    background_tasks.add_task(
        websocket_manager.broadcast_json,
        {"type": "workflow_updated", "data": workflow.to_dict()},
    )
...
async def assign_agent_to_workflow(
    ...
    workflow.add_agent(
        agent_assignment.agent_id, agent_assignment.agent_name or agent.name
    )
...
async def remove_agent_from_workflow(
    ...
    background_tasks.add_task(
        websocket_manager.broadcast_json,
        {
            ...
            "data": {"workflow_id": str(workflow.id), "agent_id": agent_id},
        },
    )
    ...
    background_tasks.add_task(
        websocket_manager.broadcast_json,
        {
            ...
            "data": {
                ...
                "scheduled_at": (
                    workflow.scheduled_at.isoformat() if workflow.scheduled_at else None
                ),
            },
        },
    )
...
async def delete_workflow(
    ...
    background_tasks.add_task(
        websocket_manager.broadcast_json,
        {"type": "workflow_deleted", "data": {"workflow_id": str(workflow_id)}},
    )
...
async def get_workflow_stats(db: AsyncSession = Depends(get_db)):
    ...
    stats = {
        ...
        "avg_progress": (
            sum(w.progress_percentage for w in workflows) / len(workflows)
            if workflows
            else 0
        ),
        "avg_success_rate": (
            sum(w.success_rate for w in workflows) / len(workflows) if workflows else 0
        ),
    }
