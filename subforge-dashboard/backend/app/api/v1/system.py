...
async def create_metrics_snapshot(
    ...
    background_tasks.add_task(
        ...
        {"type": "system_metrics_updated", "data": metrics.to_dict()},
    )
...
async def get_task_metrics(db: AsyncSession = Depends(get_db)):
    ...
    metrics = {
        ...
        "performance": {
            ...
            "avg_duration": (
                sum(t.actual_duration_minutes or 0 for t in completed_tasks)
                / len(completed_tasks)
                if completed_tasks
                else 0
            ),
...
async def get_workflow_metrics(db: AsyncSession = Depends(get_db)):
    ...
    if not workflows:
        return {"total_workflows": 0, "metrics": {}}
    ...
    metrics = {
        ...
        "performance": {
            ...
            "avg_progress": sum(w.progress_percentage for w in workflows)
            / len(workflows),
            "avg_success_rate": sum(w.success_rate for w in workflows) / len(workflows),
            "avg_duration": (
                sum(w.actual_duration_hours or 0 for w in completed_workflows)
...
async def cleanup_old_metrics(
    days_to_keep: int = 30, db: AsyncSession = Depends(get_db)
):
    """
    ...
    result = await db.execute(
        select(func.count())
        .select_from(SystemMetrics)
        .where(SystemMetrics.recorded_at < cutoff_date)
    )
    count_before = result.scalar()

    await db.execute(
        SystemMetrics.__table__.delete().where(SystemMetrics.recorded_at < cutoff_date)
    )
    await db.commit()
