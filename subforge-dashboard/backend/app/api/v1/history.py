...
async def get_workflow_history(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving workflow history: {str(e)}"
        )


@router.get("/workflows/{workflow_history_id}")
async def get_workflow_history_detail(
    workflow_history_id: str, db: AsyncSession = Depends(get_db)
):
    """
    ...
    try:
        ...
        query = (
            ...
            .where(WorkflowHistory.id == workflow_history_id)
            .options(
        ...
        workflow_data["handoffs"] = [
            handoff.to_dict() for handoff in workflow_history.handoffs
        ]
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving workflow detail: {str(e)}"
        )
...
async def get_agent_performance_history(
    ...
    period_type: Optional[str] = Query(
        None, description="Filter by period type (hour, day, week, month)"
    ),
    start_date: Optional[datetime] = Query(None, description="Start date filter"),
    end_date: Optional[datetime] = Query(None, description="End date filter"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving agent performance: {str(e)}"
        )
...
async def get_phase_history(
    db: AsyncSession = Depends(get_db),
    workflow_history_id: Optional[str] = Query(
        None, description="Filter by workflow history ID"
    ),
    agent_id: Optional[str] = Query(None, description="Filter by agent ID"),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving phase history: {str(e)}"
        )
...
async def get_handoff_history(
    ...
    agent_id: Optional[str] = Query(
        None, description="Filter by agent ID (source or target)"
    ),
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error retrieving handoff history: {str(e)}"
        )
...
async def cleanup_old_records(
    ...
    retention_days: int = Query(
        90, ge=1, le=365, description="Number of days to retain"
    ),
):
    """
    ...
    try:
        ...
        return {"message": "Cleanup completed successfully", "deleted_records": result}
...
async def calculate_agent_performance(
    ...
    try:
        ...
        raise HTTPException(
            status_code=500, detail=f"Error calculating performance: {str(e)}"
        )