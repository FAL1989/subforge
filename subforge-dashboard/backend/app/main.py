...
async def enhanced_websocket_route(websocket: WebSocket):
    ...
    try:
        while True:
            ...
            try:
                import json
                from .websocket.enhanced_manager import WebSocketMessage, MessageType
                ...
                if message_type == MessageType.JOIN_ROOM:
                    ...
                    if room_name:
                        await enhanced_websocket_manager.join_room(connection_id, room_name)
                
                elif message_type == MessageType.LEAVE_ROOM:
                    room_name = message_data.get("room")
                    if room_name:
                        await enhanced_websocket_manager.leave_room(connection_id, room_name)
                
                elif message_type == MessageType.PING:
                    await enhanced_websocket_manager.send_to_connection(
                        connection_id,
                        WebSocketMessage(type=MessageType.PONG, data={"timestamp": "now"})
                    )
