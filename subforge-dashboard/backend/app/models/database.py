...
class WorkflowHistory(Base):
    ...
    # Relationships
    phases = relationship(
        "PhaseHistory", back_populates="workflow_history", cascade="all, delete-orphan"
    )
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "trigger": {"by": self.triggered_by, "event": self.trigger_event},
...
class PhaseHistory(Base):
    ...
    agent_utilization = Column(
        Float, nullable=True
    )  # Percentage of time agent spent on this phase
    ...
    dependents = Column(
        JSON, nullable=False, default=list
    )  # Other phases depending on this
    ...
    def to_dict(self) -> dict:
        ...
        return {
            ...
            "issues": {"errors": self.error_messages, "warnings": self.warnings},
            "resource_usage": self.resource_usage,
            "created_at": self.created_at.isoformat() if self.created_at else None,
            "updated_at": self.updated_at.isoformat() if self.updated_at else None,
        }
...
class HandoffHistory(Base):
    ...
    __table_args__ = (
        Index(
            "idx_handoff_history_workflow_time", "workflow_history_id", "initiated_at"
        ),
    ...
    def to_dict(self) -> dict:
        ...
        return {
            "id": str(self.id),
            "workflow_history_id": str(self.workflow_history_id),
            "handoff": {"name": self.handoff_name, "type": self.handoff_type},
            "source": {
...
class AgentPerformanceMetrics(Base):
    ...
    __table_args__ = (
        ...
        Index(
            "idx_agent_perf_type_period", "agent_type", "period_type", "period_start"
        ),
        ...
        return {
            ...
            "performance": {
                ...
                "phases": {
                    ...
                    "failed": self.failed_phases,
                    "success_rate": (
                        self.successful_phases / self.total_phases
            ...
            "collaboration": {
                ...
                "handoff_success_rate": (
                    self.successful_handoffs
                    / (self.successful_handoffs + self.failed_handoffs)
                    if (self.successful_handoffs + self.failed_handoffs) > 0
                    else 0
                ),
            },
            "specialization": {
    ...
    def calculate_success_rates(self) -> dict:
        ...
        phase_success_rate = (
            self.successful_phases / self.total_phases if self.total_phases > 0 else 0
        )
        task_success_rate = (
            self.successful_tasks / self.total_tasks if self.total_tasks > 0 else 0
        )
        handoff_success_rate = (
            self.successful_handoffs / (self.successful_handoffs + self.failed_handoffs)
            if (self.successful_handoffs + self.failed_handoffs) > 0
            else 0
        )
        ...
        return {
            ...
            "overall_success_rate": (
                workflow_success_rate + phase_success_rate + task_success_rate
            )
            / 3,
        }
    ...
    def is_performing_well(self, threshold: float = 0.8) -> bool:
        ...
        return (
            success_rates["overall_success_rate"] >= threshold
            and self.utilization_percentage >= 0.7
        )