...
class SystemMetrics(Base):
    ...
    def calculate_derived_metrics(self):
        ...
        if total_workflows > 0:
            self.workflow_success_rate = (self.completed_workflows / total_workflows) * 100
        else:
        ...
        self.is_healthy = (
            ...
            self.system_load_percentage < 90.0 and
            self.memory_usage_percentage < 85.0
        )
    ...
    def create_snapshot(cls, agents_data=None, tasks_data=None, workflows_data=None):
        ...
        if agents_data:
            ...
            metrics.active_agents = len([a for a in agents_data if a.status == "active"])
            metrics.idle_agents = len([a for a in agents_data if a.status == "idle"])
            metrics.offline_agents = len([a for a in agents_data if a.status == "offline"])
        
        if tasks_data:
            metrics.total_tasks = len(tasks_data)
            metrics.pending_tasks = len([t for t in tasks_data if t.status == "pending"])
            metrics.in_progress_tasks = len([t for t in tasks_data if t.status == "in_progress"])
            metrics.completed_tasks = len([t for t in tasks_data if t.status == "completed"])
            metrics.failed_tasks = len([t for t in tasks_data if t.status == "failed"])
        
        if workflows_data:
            metrics.total_workflows = len(workflows_data)
            metrics.active_workflows = len([w for w in workflows_data if w.status == "active"])
            metrics.completed_workflows = len([w for w in workflows_data if w.status == "completed"])
            metrics.paused_workflows = len([w for w in workflows_data if w.status == "paused"])
