...
class WorkflowCreate(WorkflowBase):
    ...
    status: str = Field(
        default="active", regex="^(active|inactive|paused|completed|failed|cancelled)$"
    )
...
class WorkflowUpdate(BaseModel):
    ...
    def validate_completed_tasks(cls, v, values):
        ...
        if (
            v is not None
            and "total_tasks" in values
            and values["total_tasks"] is not None
        ):
...
class WorkflowStatusUpdate(BaseModel):
    ...
    status: str = Field(
        ..., regex="^(active|inactive|paused|completed|failed|cancelled)$"
    )
    progress_percentage: Optional[float] = Field(None, ge=0.0, le=100.0)
