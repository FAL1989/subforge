...
class TaskCreate(TaskBase):
    ...
    status: str = Field(default="pending", regex="^(pending|in_progress|completed|failed|blocked|cancelled)$")
...
class TaskUpdate(BaseModel):
    ...
    description: Optional[str] = None
    status: Optional[str] = Field(None, regex="^(pending|in_progress|completed|failed|blocked|cancelled)$")
...
class TaskStatusUpdate(BaseModel):
    ...
    status: str = Field(..., regex="^(pending|in_progress|completed|failed|blocked|cancelled)$")
    progress_percentage: Optional[float] = Field(None, ge=0.0, le=100.0)
    quality_score: Optional[float] = Field(None, ge=0.0, le=100.0)
...
class TaskProgressUpdate(BaseModel):
    ...
    def validate_subtasks_completed(cls, v, values):
        ...
        if v is not None and "subtasks_total" in values and values["subtasks_total"] is not None:
