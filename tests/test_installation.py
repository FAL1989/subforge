...
class TestInstallation:
    ...
    def test_templates_are_included(self):
        ...
        try:
            ...
            assert templates_path.is_dir(), f"Templates path is not a directory: {templates_path}"
            
            # Check for at least one template file
            template_files = list(templates_path.glob('*.md'))
            assert len(template_files) > 0, f"No template files found in {templates_path}"
