...
"""
    ...
    async def _run_template_selector(
        ...
        # First, use ALL recommended agents from project analysis
        selected_templates = []
        available_templates = list(self.subagent_executor.available_subagents.keys())
        
        print(f"      📋 Found {len(context.project_profile.recommended_subagents)} recommended agents")
        
        # Add all recommended agents that exist as templates
        for agent in context.project_profile.recommended_subagents:
            if agent in available_templates:
                selected_templates.append(agent)
                print(f"      ✅ Added recommended agent: {agent}")
            else:
                print(f"      ⚠️  Recommended agent '{agent}' template not found, skipping")

        # Then add high-scoring templates not already selected (increase limit to 12)
        for match in template_matches:
            if match["template"] not in selected_templates and match["score"] > 0.3 and len(selected_templates) < 12:
                selected_templates.append(match["template"])
                print(f"      ➕ Added high-scoring template: {match['template']} (score: {match['score']:.3f})")
        ...
        for agent in essential_agents:
            if agent not in selected_templates:
                selected_templates.append(agent)
                print(f"      🔧 Added essential agent: {agent}")

        # Generate detailed selection report
        recommended_count = len([t for t in selected_templates if t in context.project_profile.recommended_subagents])
        
        selection_report = f"""# Intelligent Template Selection Report

## Selection Summary
- **Total Templates Evaluated**: {len(template_matches)}
- **Recommended Agents Used**: {recommended_count} out of {len(context.project_profile.recommended_subagents)}
- **Selection Strategy**: Recommended agents first, then high-scoring additions
- **Final Team Size**: {len(selected_templates)} agents

## Selected Agent Categories

### Recommended Agents (From Project Analysis)
{chr(10).join(f'- {agent}' for agent in selected_templates if agent in context.project_profile.recommended_subagents)}

### Additional High-Scoring Agents
{chr(10).join(f'- {agent}' for agent in selected_templates if agent not in context.project_profile.recommended_subagents and agent not in ["orchestrator", "code-reviewer", "test-engineer"])}

### Essential Agents (Always Included)
{chr(10).join(f'- {agent}' for agent in ["orchestrator", "code-reviewer", "test-engineer"] if agent in selected_templates)}

## Top Template Matches (Reference)

{chr(10).join(f'''### {i+1}. {match["template"]} (Score: {match["score"]:.3f}) {'✅ SELECTED' if match["template"] in selected_templates else ''}
**Match Reasons**:
{chr(10).join(f'- {reason}' for reason in match["reasons"])}
''' for i, match in enumerate(template_matches[:10]))}

## Team Composition Analysis
**Coverage**: All major project areas covered
**Balance**: Optimal agent distribution based on project analysis
**Specialization**: Each agent has clear domain ownership

Generated by SubForge Template Selector Factory Agent (Enhanced)
"""
...
"""
    ...
    async def _handle_selection_phase(
        ...
        print(f"    📋 Using {len(context.project_profile.recommended_subagents)} recommended agents from analysis")
        ...
        # Then add high-scoring templates not already selected (up to 12 total)
        for match in template_matches:
            if match["template"] not in selected_templates and match["score"] > 0.3 and len(selected_templates) < 12:
                selected_templates.append(match["template"])
                print(f"    ➕ Added high-scoring template: {match['template']} (score: {match['score']:.3f})")
