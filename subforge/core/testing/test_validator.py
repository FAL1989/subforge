...
class TestValidator:
    ...
    def validate_test_case(self, test_case) -> ValidationResult:
        ...
        passed = all(issue.severity not in [ValidationSeverity.CRITICAL, ValidationSeverity.ERROR] 
                    for issue in issues)
    ...
    def _validate_test_structure(self, test_case, tree) -> List[ValidationIssue]:
        ...
        for node in ast.walk(tree):
            if isinstance(node, ast.FunctionDef) and node.name.startswith('test_'):
                ...
                comments = self._extract_comments(test_case.code, node.lineno, node.end_lineno)
                has_aaa_comments = any(
                    comment.lower().strip() in ['# arrange', '# act', '# assert'] 
                    for comment in comments
                )
                
                if not has_aaa_comments and self.validation_level == ValidationLevel.COMPREHENSIVE:
                    issues.append(ValidationIssue(
                        severity=ValidationSeverity.WARNING,
    ...
    def _validate_assertions(self, test_case, tree) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
    ...
    def _validate_suite_structure(self, test_suite) -> Tuple[List[ValidationIssue], Dict[str, Any]]:
    ...
    def _calculate_test_score(self, issues: List[ValidationIssue], metrics: Dict[str, Any]) -> float:
    ...
    def _generate_recommendations(self, issues: List[ValidationIssue], metrics: Dict[str, Any]) -> List[str]:
        ...
        if issue_counts.get('critical', 0) > 0:
            recommendations.append("Fix critical issues immediately - tests may not run")
        
        if issue_counts.get('error', 0) > 0:
            recommendations.append("Address error-level issues to improve test reliability")
        ...
        if total_tests > 0:
            ...
            if unit_percentage < 60:
                recommendations.append("Increase unit test coverage - aim for 60-80% unit tests")
        
        return recommendations
    
    def _generate_test_recommendations(self, issues: List[ValidationIssue], test_case) -> List[str]:
        ...
        if 'ASSERTION_COUNT' in issue_types or 'ASSERTION_MISSING' in issue_types:
            recommendations.append("Add more comprehensive assertions to verify behavior")
        
        if 'NAMING_DESCRIPTIVE' in issue_types:
            recommendations.append("Use more descriptive test names that explain the scenario")
        ...
        if 'DOC_MISSING_DOCSTRING' in issue_types:
            recommendations.append("Add docstrings to explain test purpose and expected outcomes")
