...
"""
    ...
    def _format_examples_as_markdown(self) -> str:
        ...
        for i, example in enumerate(self.examples, 1):
            sections.append(f"""### Example {i}: {example.get('title', 'Untitled')}
...
""")
    ...
    def _format_validation_gates_as_markdown(self) -> str:
        ...
        for gate in self.validation_gates:
            gates.append(f"""### {gate.get('name', 'Unnamed Gate')}
...
class ContextEngineer:
    ...
    def engineer_context_for_phase(
        ...
        """
        print(f"    🎯 Engineering {context_level.value} context for {phase_name} phase...")
        ...
        print(f"    ✅ Context package created: {len(examples)} examples, {len(patterns)} patterns")
    ...
    def _build_technical_context(self, profile: ProjectProfile, phase_name: str) -> Dict[str, Any]:
        ...
        if phase_name == "analysis":
            base_context.update({
                "analysis_depth": "deep" if profile.complexity.value != "simple" else "standard",
                "discovery_areas": ["architecture", "dependencies", "patterns", "performance"]
            })
        elif phase_name == "selection":
            base_context.update({
                "template_criteria": self._get_template_criteria(profile),
                "customization_level": "high" if profile.complexity.value == "complex" else "medium"
            })
        elif phase_name == "generation":
            base_context.update({
                "generation_targets": ["claude_md", "agents", "workflows", "commands"],
    ...
    def _find_relevant_examples(self, profile: ProjectProfile, phase_name: str) -> List[Dict[str, Any]]:
        ...
        arch_examples = self._get_architecture_examples(profile.architecture_pattern, phase_name)
        examples.extend(arch_examples)
        
        return examples[:5]  # Limit to most relevant
    
    def _get_language_examples(self, language: str, phase_name: str) -> List[Dict[str, Any]]:
...
""",
    ...
    def _get_framework_examples(self, framework: str, phase_name: str) -> List[Dict[str, Any]]:
...
""",
    ...
    def _get_architecture_examples(self, architecture, phase_name: str) -> List[Dict[str, Any]]:
...
"""
    ...
    def _extract_patterns(self, profile: ProjectProfile, phase_name: str, context_level: ContextLevel) -> List[Dict[str, Any]]:
    ...
    def _get_advanced_patterns(self, profile: ProjectProfile, phase_name: str) -> List[Dict[str, Any]]:
        ...
        if phase_name == "generation":
            patterns.append({
                ...
                "benefits": ["Clearer responsibilities", "Better context", "Reduced conflicts"],
                "examples": ["api-developer for REST APIs", "database-specialist for data models"]
            })
        
        return patterns
    
    def _get_expert_patterns(self, profile: ProjectProfile, phase_name: str) -> List[Dict[str, Any]]:
        ...
        if profile.complexity.value == "complex":
            patterns.append({
                ...
                "examples": ["Formal code review workflows", "Multi-stage validation"]
            })
        
        return patterns
    
    def _create_validation_gates(self, phase_name: str, profile: ProjectProfile) -> List[Dict[str, Any]]:
    ...
    def _define_success_criteria(self, phase_name: str, profile: ProjectProfile) -> List[str]:
    ...
    def _get_template_criteria(self, profile: ProjectProfile) -> List[str]:
        ...
        criteria.append(f"Support for {', '.join(profile.technology_stack.languages)} languages")
        criteria.append(f"Compatible with {profile.architecture_pattern.value} architecture")
