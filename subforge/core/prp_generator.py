...
from .context_engineer import ContextPackage, ContextLevel
...
"""
    ...
    def _get_subagent_execution_prompt(
        ...
        if subagent_type == "claude-md-generator":
            return self._get_claude_md_generator_prompt(project_profile, context_package, analysis_outputs)
        elif subagent_type == "agent-generator":
            return self._get_agent_generator_prompt(project_profile, context_package, analysis_outputs)
        elif subagent_type == "workflow-generator":
            return self._get_workflow_generator_prompt(project_profile, context_package, analysis_outputs)
        else:
            return self._get_generic_subagent_prompt(subagent_type, project_profile, context_package, analysis_outputs)
...
"""
    ...
    def _format_analysis_insights(self, analysis_outputs: Dict[str, Any]) -> str:
        ...
        for key, value in analysis_outputs.items():
            if isinstance(value, dict):
                insights.append(f"**{key.replace('_', ' ').title()}**: {len(value)} items identified")
            elif isinstance(value, list):
                insights.append(f"**{key.replace('_', ' ').title()}**: {', '.join(map(str, value[:3]))}")
            else:
                insights.append(f"**{key.replace('_', ' ').title()}**: {value}")
        
        return '\n'.join(insights) if insights else "Standard analysis approach will be used"
...
- **Monitoring**: Application and system monitoring"""
    ...
    def _get_subagent_success_metrics(self, subagent_type: str) -> List[str]:
        ...
        return ["Generation completed within expected timeframe", "Output quality meets standards"]
    ...
    def _get_subagent_output_specification(self, subagent_type: str) -> Dict[str, Any]:
        ...
        if subagent_type == "claude-md-generator":
            return {
                ...
                "format": "markdown",
                "sections_required": [
                    "project_overview", "build_commands", "code_style", 
                    "architecture", "workflow", "agent_team"
                ],
        ...
        return {"primary_output": f"{subagent_type}_configuration", "format": "markdown"}
